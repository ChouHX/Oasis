...
python3 tests/test_merge.py              # 上一版记录的翻译（预约 ≠ 中签）
...
| 文件 | 覆盖什么 | 联网 |
| --- | --- | --- |
| `test_hitcheck.py` | **截止之后**的 Oasis 来信才算中签；截止之前收到的「结果信」不算；Registration Complete 只是预约成功；验证信不算；别家邮件不算；`confirm OK 无邮件` 与页面确认只推断成「已预约」；读信失败只记错误；旧痕迹推断为已预约；别名分组；首次全量其后粗筛；第二轮不改变已有结论 | 否 |
| `test_merge.py` | 上一版记录的翻译：`registered` / `submitted` / 那条 `confirm answered OK…` / 有预约行的，全部只翻译成「已预约」，**不是中签**；旧的误判 `hit_at` 被撤销、时间搬到 `registered_at`；真正的失败不并入；重复打开不改结论；默认截止时间解析为 2026-09-17 15:00 UTC | 否 |

...
服务端带一个网页管理界面，五个页面：**仪表盘 / 邮箱池 / 代理池 / 预约记录 / 设置**。
日志并在仪表盘下方——打开就是「统计 + 运行控制 + 实时日志」一屏，不用切页。

日志轮询是**常驻**的（启动即开，2 秒一次），而不是切到某个标签才启动。页面里最多
保留 1500 行 DOM，环形缓冲本身存 2000 行，避免长跑之后滚动变卡。**只用标准库**，不引额外依赖，镜像不变大。

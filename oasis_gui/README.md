...
池子里大多数地址没预约过 —— 试过但没成功、或者只是被存着 —— 而没预约过的邮箱
不可能收到中签信，扫它只是白花时间。所以检测范围默认只含 `opted_in=1` 的账号，
「预约过」的证据来自三处：

1. **上一版程序留下的**，打开库时自动并入：状态是 `registered` / `submitted`、
   错误里含那条 `confirm answered OK but the success mail never arrived`、
   或者有 `registrations` 行。
2. **导入时的勾选**（默认打开）—— 粘一批地址进来，图的本来就是「查这些」。
3. **邮箱池页的手工标记**：勾几行点「所选标为已预约」，或者一次把全部未标记的
   标进来。用别的工具或手工预约过的地址，只有这一条路 —— 程序推不出来，但操作者
   知道。

统计口径也随之分两层：**账号总数**是池子规模，**已预约**是检测范围；
「还没查过 / 查过未中签 / 读信失败」三个进度数都只在已预约里算。以前它们按全体
算，于是「还没查过 453」看起来像进度落后，其实分母里有一半地址根本不会被查。
关掉设置里的「只查已预约」就退回整池都查 —— 只有在「池子里哪些地址预约过、心里
没底」时才需要那样。
...
- 实测效果：混着无关邮件的账号，读取从 7 封降到 4 封 / 1 封，而 Oasis 的信一封
  ...
  就关掉它。
- 回看窗口（`lookback_days`）也一并下推：Graph 用 `receivedDateTime ge`，IMAP 用
  `SINCE`。以前它只在本地判定时起作用，同样的历史信每轮都被重新下载一遍。
- 账号池里没有能走 IMAP 的邮箱（Outlook 对这些 OAuth 号回 "User is authenticated
  but not connected"），所以 IMAP 那一侧只验到语法串与降级路径；Graph 那一侧是
  真机验的 —— 用不可能满足的窗口条件（未来时间）确认 `$filter` 确实在服务端跑。
...
这是现在唯一的失败模式。查下来**问题不在我们**，证据分三层：

**① 字段接受任何输入。** 用一封邮件链接连试 8 个号码，全部通过：

```
7125557686  4122650944  2125551234  4155551234
5555555555  0005551234  2120111234  212555123（只有 9 位）
```

连 `0005551234` 和 9 位数都接受，所以站点根本没在校验号码格式，更不是区号与
地址是否同州。

**② 后面的步骤不会破坏它。** 对照了三种填法——只填电话 / 填完生日 /
再填完地址——4 次全部通过。所以也不是填地址把电话状态搞坏了。

**③ YG 手动填表时也遇到过同样的提示。** 那就排除了「我们的自动化输入方式」。

统计历史日志：**26 次 details 填充里 5 次报这个错，约 19%**。总结：**站点自己的
间歇性拒绝**，和账号、号码、输入方式都无关。

### 应对：三步都试过，最后一条才是有效的

| 做法 | 效果 |
| --- | --- |
| 重试同一个号码 12 次 | **无效**——同值重试收益为零，还花掉 120 秒 |
| 重试时换新号码 | 略好，但字段本来就接受任何号码，属于巧合 |
| **重试前重新加载页面** | **有时有效**——探针每次重载都不复现，说明错误提示会粘在组件上 |
| **重试耗尽后按瞬时错误退回队列** | **有效**——实测一个账号失败两次后重新领回，第 3 次成功 |

最后一条是关键：既然站点会间歇性抽风，账号就不该因此报废。现在
`details step` 重试 2 轮仍失败就抛 `TransientError`，由引擎退回队列
（每轮传输失败有预算，不会无限循环）。实测 3 个账号 **3/3 成功、零报废**。
...
`Please enter a valid phone number` 是现在唯一的失败模式，而且**重试同一个号码从来
不成功**——实测同一个身份连试 12 次（字段级 4 次 × 步骤级 3 次）全部失败，号码本身
却是合法的 NANP 号。

所以两处改动：

- 重试时**换一个新号码**（`identity.random_phone`），而不是重复原值。实测：改前
  3 个账号只过 1 个，改后 3/3
- 整步重试从 3 轮降到 2 轮。12 次尝试换来 120 秒的浪费，而同一值重试的收益是零

根因还没查清（可能是组件的输入事件偶尔没触发），但重试换值这条路是有效的。
...
这些都是实测逼出来的，记下来是因为它们都不是「看起来会出错」的那种：

- **凭据曾经每 3 秒上一次网**。`/api/state` 是页面轮询的接口，它把整个配置原样
  发回浏览器 —— 包括 iCloud 密码、别名收件箱的应用专用密码、带 `user:pass` 的
  取件代理。现在凭据只报「有没有设置」，代理只报 `***:***@host`，而「留空 = 不
  改」由服务端兜住（页面回传遮罩值不会把密码写成三个星号）。
- **一个读不通的邮箱曾把整轮拖 8.5 分钟**，而且 `docker stop` 要等它 —— 因为
  `ThreadPoolExecutor` 的线程不是 daemon 的，解释器退出时会 join 它们。现在自己
  开 daemon 线程，有卡住的读信时 SIGTERM 到进程消失是 0.5 秒。
- **「下一轮优先重试」曾经是句假话**：放弃等待之后慢线程还会回来写一次
  `checked_at`，把账号推到队尾去了。现在被放弃的账号只写结论、不推进进度。
- **服务端拒绝粗筛曾等于「这个邮箱没有来信」**，静默返回空。现在抛
  `MailSearchUnsupported` 并退回全量。
- **别名按 `client_id` 分组的空值会被拼在一起**，等于拿 A 的收件箱密码去登 B 的
  别名。缺 `client_id` 的行现在各自成组。
- **debug 日志曾无差别打到 stdout**，每个账号一条能把要紧的几行淹掉。现在按
  `debug` 开关过滤。

...
## 判定注册是否成功：只看邮箱，别信响应### 浏览器模式 = 驱动官方表单，不是自己拼请求
...
服务端带一个网页管理界面，**按桌面端复刻**：左侧导航栏、同样的六个页面、
同样的 Fluent 2 视觉规范。

```
┌───────────┬────────────────────────────────────────┐
│ 仪表盘     │  ┌ Oasis Live '27 注册控制台 ────────┐   │
│ 邮箱池     │  └──────────────────────────────────┘   │
│ 代理池     │  [账号总数][待注册][进行中][邮件确认]…    │
│ 预约记录   │  ┌ 运行控制 ─────────────┐               │
│ 数据库     │  └──────────────────────┘               │
│           │  ┌ 实时日志 ─────────────┐               │
│ ─────────  │  └──────────────────────┘               │
...
用的是内联的一套 Fluent 2 设计令牌，而不是从 unpkg/jsdelivr 拉 `@fluentui/*`。
原因是**这是个自托管工具**：CDN 在国内经常慢或不通，一旦拉不下来整个管理界面
就白屏，而它还得能管队列、看日志。内联之后零外部依赖，界面永远能开。

视觉上对齐桌面端（qfluentwidgets）的具体取值：

| 项 | 取值 |
| --- | --- |
| 导航 | 左侧栏 190px，选中项左侧 3px 强调色竖条；「设置」用分隔线钉在底部 |
| 统计卡 | 64px 高，**大号彩色数字在上、灰色小标签在下**（与桌面端 StatCard 一致） |
| 卡片 | 8px 圆角，带标题头（36px）+ 内容区，对应 `HeaderCardWidget` |
| 按钮 | **28px 高**、4px 圆角；主操作强调色填充，次要项描边 |
| 横幅 | 橙色渐变 `#b4531f → #d9822b → #f2c14e`，72px，与桌面端 Banner 同款 |
| 字体 | `Segoe UI Variable Text` → `Segoe UI` → 系统字体 |
| 主题 | 跟随系统深浅色；浅色底 `#f3f3f3`／强调 `#005fb8`，深色底 `#202020`／强调 `#60cdff` |

统计卡的强调色直接用桌面端那组（`#3b7dd8` 账号总数、`#1f9d55` 邮件确认、
`#3fa88a` 页面确认、`#d64545` 失败……），两边的色彩语义一致。

窄屏（< 820px）时左侧栏转为顶部横向滚动条，手机上也能用。
